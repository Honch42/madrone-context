...
* **Bring-Your-Own-Key, with discovery:** keys already on the Mac (login-shell environment, `~/.gemini/.env`, `~/.env`, Claude Code settings, Codex CLI auth, the earlier Keychain entries) are found and offered with one click, masked, and never used without the user choosing them. An Anthropic CLI sign-in (`ant auth login`) can be used for Claude models instead of a key. Keys are encrypted with Electron's `safeStorage` (macOS Keychain).
