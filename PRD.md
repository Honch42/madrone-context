...
* Contexts (multiple notes folders or vaults, each with its own dossier and graph) and the phone-capture inbox with a guided review that files to-dos and thoughts into the right context.
...
### 11.7a Contexts and the Inbox
* **Contexts.** The app supports any number of contexts, each a name plus a notes folder (a whole vault or a folder inside one). Each has its own `Madrone/` folder, Master Dossier, sessions, entity notes and Bases table. The context is chosen in the top bar before a session; an install from before contexts existed becomes a single context automatically.
* **Per-context Google account scoping.** Google sign-in and Screenpipe stay global, machine-level connections — reconnecting per context would be needless friction, since the underlying accounts and local database don't change per area of work. What each context *sees* of them does: each context can be scoped to a subset of the connected Google accounts (none, one, several, or the default of every account including ones connected later), set as checkboxes in Settings. `config.googleAccountsForContext(id)` resolves the scope, and `gatherRearwardContext`/`gatherForwardContext` in the server pass a session's resolved accounts through to `fetchRearwardContext`/`fetchForwardContext` in `src/google.js`, which query only those. A scope naming accounts that have since been disconnected falls back to every remaining account rather than silently returning nothing. Screenpipe has no equivalent scoping since it is one local database per Mac with nothing to choose between.
* **Inbox review.** One or more inbox folders hold captures from the phone (voice memos and quick notes). "Review inbox" is a second session type: iCloud placeholders are downloaded, voice memos transcribed by Gemini Flash, and the interviewer walks through the items in order, reading each back and asking one clarifying question when needed. The model's reply carries structured decisions (`todo`, `thought` or `discard`; the target context; title, clarified text, optional due date, and people/projects/topics), which the app applies immediately: to-dos append to `Madrone/Action Items.md` in Obsidian Tasks format, thoughts to a dated `Madrone/Thoughts/` note, entity notes are created in the target context, and the file moves to `Inbox/Processed/`. The review's own note, with every decision, is saved in the context the user started from. This is the first delivery of the PRD's "Ad-Hoc / Mid-Day Context Dump" and the capture half of the evening debrief.

### 11.8 Distribution
A GitHub Actions workflow (`.github/workflows/build-mac.yml`) builds the app on GitHub's macOS runners: it installs dependencies, runs the smoke test, builds a universal `.dmg` (Apple Silicon and Intel), and publishes a GitHub Release when run with a version or when a `v*` tag is pushed. Google sign-in is included when the `GOOGLE_OAUTH_CLIENT_JSON` repository secret exists; Apple signing and notarization switch on when the certificate and Apple ID secrets exist. Without them the build is unsigned and, on first launch, users click **Open Anyway** in System Settings > Privacy & Security once (macOS 15 removed the older right-click > Open shortcut). **Release v1.0.0** was built and published this way on September 15, 2026, in under four minutes; the download is https://github.com/Honch42/madrone-context/releases/latest. Building locally with `npm run build` still works.
